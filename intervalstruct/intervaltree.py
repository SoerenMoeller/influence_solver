...
"""
Changed:
- overlap/envelop method fixed finding index
- overlap/envelop method empty interval condition
- added overlap_exclusive to remove neighbor intervals
"""
...
class IntervalTree(MutableSet):
    ...
    def envelop(self, begin, end=None):
        ...
        bound_end = boundary_table.bisect_right(end)  # up to, but not including end
    ...
    def overlap_exclusive(self, begin, end):
        overlapping: set = self[begin:end]

        return {interval for interval in overlapping if not interval.end == begin and not interval.begin == end}

    def overlap(self, begin, end=None) -> list:
        """
        ...
        boundary_table = self.boundary_table
        bound_begin = boundary_table.bisect_left(begin)
        bound_end = boundary_table.bisect_right(end)  # up to, but not including end # CHANGED
        result.update(root.search_overlap(
            # slice notation is slightly slower
            boundary_table.keys()[index] for index in xrange(bound_begin, bound_end)
        ))
        return sorted(result)       # CHANGED: sorted
